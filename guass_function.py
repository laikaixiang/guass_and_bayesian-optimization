...
def guassFitData(guass_number,
    ...
    # bounds: 参数的范围，可以自定义，
            # 默认为bounds = ([0 for i in range(guass_number * 3)],[np.inf for i in range(guass_number * 3)])
            # 输入应该按照参考格式来输入， 顺序是（[均值]，[标准差]，[系数]）* 高斯函数峰数量
    ...
    def get_params():
        ...
        for i in range(guass_number): # 随机数调整
            initial_params[i * 3] = min(xdata)+(i+1)/guass_number*(max(xdata)-min(xdata))
            initial_params[i * 3 + 1] = np.random.rand() * 200
            initial_params[i * 3 + 2] = np.random.rand() * 0.5 * max(ydata[:,0])
    ...
    for index in range(ydata.shape[1]):
        ...
        if save_and_view_training_path == None:
            ...
            plt.title('Least Squares Fit of Gaussian Sum of Metrial ' + ytitle[index][0])
            plt.legend()
    ...
    finial_params = pd.DataFrame(finial_params)
    ...
    if save_and_view_training_path != None:
        ...
        finial_params.to_csv(R2_path, encoding='utf-8')
...
if __name__ == "__main__":
    ...
    guassFitData(2, "代表性光谱.csv", save_params_path="拟合参数第二轮.csv", save_and_view_training_path="image")